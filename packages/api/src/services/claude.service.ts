...
${text}`;
  ...
  /**
   * Generate 3 Plaud-style pro summaries for a meeting transcript.
   * Returns { notes, qa, actions } — each is markdown text.
   */
  async generateProSummaries(transcript: string, title: string, people: string[]): Promise<{ notes: string; qa: string; actions: string }> {
    const context = `Название встречи: ${title}\nУчастники: ${people.join(', ') || 'не указаны'}\n\nТранскрипция:\n${transcript.slice(0, 30000)}`;

    const [notes, qa, actions] = await Promise.all([
      // 1. NOTES — structured meeting notes
      this.chat([{ role: 'user', content: `Ты — профессиональный аналитик встреч. Создай подробные структурированные ЗАМЕТКИ по встрече в стиле Plaud/Otter.ai.

ФОРМАТ (markdown):
# [Название встречи]

## Предпосылки и основная информация
[2-3 абзаца: кто участвовал, какие компании представляют, чем занимаются, контекст встречи]

## [Тема 1]: [название]
[Подробное описание обсуждения этой темы. Конкретные цифры, факты, стратегии. Упоминай участников через [Speaker N] или по имени.]
...
## План
[Список конкретных задач с чекбоксами, ответственными и сроками]
- [ ] Задача — [Ответственный] [срок если есть]

ВАЖНО:
- Пиши на русском
- Будь конкретен: цифры, названия компаний, суммы, сроки
- Каждую тему раскрывай подробно (5-10 предложений минимум)
...
## План
[Чекбоксы задач с ответственными]
- [ ] Задача — [Ответственный]

ВАЖНО:
- 5-15 вопросов, покрывающих все темы встречи
- Каждый ответ 3-8 предложений с конкретикой
- Пиши на русском
- Формулируй вопросы так, как спросил бы человек, который не был на встрече

${context}` }], '', 'gpt-4.1'),

      // 3. ACTIONS — client needs / action items
      this.chat([{ role: 'user', content: `Ты — профессиональный бизнес-аналитик. Создай документ ПОТРЕБНОСТИ И ДЕЙСТВИЯ по встрече.

ФОРМАТ (markdown):
# [Название встречи] — Анализ

## Обзор
[Краткое описание встречи, участников, целей]

## Справочная информация
[Детальная информация о компаниях/проектах участников: чем занимаются, история, ключевые цифры]

## Болевые точки
[Список проблем и вызовов, которые обсуждались. Каждый пункт — 2-3 предложения с конкретикой]

## Ожидания
[Что участники хотят получить / к чему стремятся]
...
ВАЖНО:
- Пиши на русском
- Будь конкретен: цифры, названия, суммы
- Каждый раздел 3-5 пунктов минимум
- Анализируй глубоко, не поверхностно

${context}` }], '', 'gpt-4.1'),
    ]);

    return { notes, qa, actions };
  }
