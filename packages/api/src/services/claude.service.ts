...
${text}`;
  ...
  async generateProSummaries(transcript: string, title: string, people: string[]): Promise<{ notes: string; qa: string; actions?: string }> {
    ...
    const [notes, qa] = await Promise.all([
      // 1. NOTES — full structured meeting notes with conclusions and tasks
      this.chat([{ role: 'user', content: `Ты — профессиональный аналитик встреч. Создай ПОЛНЫЕ подробные структурированные ЗАМЕТКИ по встрече в стиле Plaud/Otter.ai.
...
${context}` }], '', 'gpt-4.1-mini'),
    ...
    // 3. Extract unique tasks from both notes and Q&A — deduplicate
    let actions: string | undefined;
    try {
      const actionsResp = await this.chat([{ role: 'user', content: `Из двух документов ниже извлеки ВСЕ уникальные задачи и обязательства. Убери дубли. Каждая задача — одна строка с чекбоксом.
