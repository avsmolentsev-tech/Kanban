...
describe('телефоны: границы и дополнительные форматы (найдено ревью)', () => {
  ...
  // Задача 4 (реальная утечка): 11-значный номер с голым "7" впереди без "+" не
  // попадал ни под PHONE_RU (требовал литеральный "+7"/"8"), ни под PHONE_RU_BARE
  // (границу (?<!\d) перед "9" блокировала предшествующая цифра "7") — самый
  // частый в CRM-выгрузках и веб-формах формат номера уходил в модель в открытом
  // виде. redactPii('Контакты. Телефон 79161234567') раньше возвращал текст БЕЗ
  // изменений — проверено до правки.
  test('11-значный номер с голым "7" впереди («79161234567») ловится', () => {
    const r = redactPii('Контакты. Телефон 79161234567');
    expect(r.text).not.toContain('79161234567');
    expect(r.text).toBe('Контакты. Телефон [ТЕЛЕФОН_1]');
  });

  test('11-значный номер с голым "7", продиктованный по группам («7 916 123 45 67»), ловится', () => {
    const r = redactPii('Мой номер 7 916 123 45 67, звоните');
    expect(r.text).not.toContain('916');
    expect(r.text).toBe('Мой номер [ТЕЛЕФОН_1], звоните');
  });

  // Пин границы (задача 4): расширение префикса PHONE_RU на голый "7" не должно
  // ослаблять guard (?<!\d)/(?!\d) — оба репродукса из отчёта ревью повторно
  // проверены явно рядом с новым форматом, а не только в старом тесте выше.
  test('граница не ослаблена: длинный слитный хвост и банковский счёт по-прежнему не матчатся частично', () => {
    const order = 'Заказ 89161234567890 в обработке';
    const account = 'счёт 40817810099910004312';
    expect(redactPii(order).text).toBe(order);
    expect(redactPii(account).text).toBe(account);
  });
});
