 ...
 * то, что док-комментарии функции прямо запрещают. Тогдашняя починка сузила
 * регулярку ТЕЛЕФОН лукэхедом по длине числа (1–3 цифры без продолжения).
 *
 * Задача 4 (пятое ревью): к этому моменту риск утечки текстом уже закрыт
 * структурно (четвёртое ревью) — функция возвращает только `{kind, index}`,
 * никогда сам текст совпадения. Лукэхед по длине числа с тех пор не защищает
 * ни от чего: избыточное совпадение — это просто лишняя запись в результате
 * (кому/куда), а не утечка значения. При этом он тихо терял часть настоящих
 * искажений («ТЕЛЕФОН_89161234567», «телефон_1234», второй «Телефон N» в
 * списке из нескольких пунктов). Лукэхед убран, ТЕЛЕФОН ведёт себя как EMAIL и
 * УЧАСТНИК. Ниже — те же репродуксы, что и раньше, с перевёрнутым ожиданием:
 * раньше «не токен», теперь «находится, но результат не содержит текста».
 */
describe('findResidualPiiTokens: длина числа после ТЕЛЕФОН больше не ограничена (задача 4, снятие лукэхеда)', () => {
  test('«телефон» строчными + пробел + настоящий слитный номер — снова находится как искажённый токен', () => {
    expect(findResidualPiiTokens('оставила телефон 89161234567')).toEqual([{ kind: 'ТЕЛЕФОН', index: 9 }]);
  });

  test('«Телефон» с заглавной, номер продиктован по группам через пробел — снова находится', () => {
    expect(findResidualPiiTokens('Телефон 8 916 123 45 67')).toEqual([{ kind: 'ТЕЛЕФОН', index: 0 }]);
  });
  ...
  // Задача 4 (пятое ревью): раньше этот тест проверял, что restorePiiAndWarn НЕ
  // логирует остаток на обычной фразе «телефон + слитный номер» — именно ради
  // этого и существовал лукэхед по длине числа. После снятия лукэхеда фраза
  // снова считается похожей на искажённый токен и warn ВЫЗЫВАЕТСЯ — но, в
  // отличие от версии до четвёртого ревью, утечки значения из карты в этом нет:
  // строка предупреждения содержит только класс и позицию, реального номера в
  // ней нет структурно. Было: `expect(warnSpy).not.toHaveBeenCalled()`.
  // Стало: warn вызывается один раз, а в строке нет ни реального номера, ни
  // вообще числового прогона длиннее позиции индекса.
  test('restorePiiAndWarn логирует остаток на обычной фразе «телефон + слитный номер», но без значения из карты', () => {
  ...
  // Задача 4: длинный слитный номер и номер, продиктованный по группам, теперь
  // тоже находятся как искажённая форма (снятие лукэхеда, см. выше) — раньше
  // это было ограничением, которое эти же тесты и фиксировали в перевёрнутом виде.
  test('«ТЕЛЕФОН_89161234567», «телефон_1234», «Телефон 1» в списке из нескольких пунктов — все находятся', () => {
    expect(findResidualPiiTokens('ТЕЛЕФОН_89161234567')).toEqual([{ kind: 'ТЕЛЕФОН', index: 0 }]);
    expect(findResidualPiiTokens('телефон_1234')).toEqual([{ kind: 'ТЕЛЕФОН', index: 0 }]);
    expect(findResidualPiiTokens('ТЕЛЕФОН 1234')).toEqual([{ kind: 'ТЕЛЕФОН', index: 0 }]);
    expect(findResidualPiiTokens('Телефон 1 2 вопроса')).toEqual([{ kind: 'ТЕЛЕФОН', index: 0 }]);
    const list = findResidualPiiTokens('1. Телефон 1\n2. Телефон 2 не отвечает');
    expect(list).toHaveLength(2);
    expect(list.map((r) => r.kind)).toEqual(['ТЕЛЕФОН', 'ТЕЛЕФОН']);
  });
});
