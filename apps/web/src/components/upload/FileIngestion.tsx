  ...
  return (
    <div className="space-y-4">
      ...
      <div onDragOver={(e) => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)}
        ...
        <input ref={fileRef} type="file" className="hidden" accept=".txt,.md,.pdf,.docx" onChange={(e) => { const f = e.target.files?.[0]; if (f) processFile(f); }} />
        <div className="text-gray-500 text-sm">{loading ? 'Processing...' : 'Drop a file here or click to upload'}</div>
        <div className="text-gray-400 text-xs mt-1">Supported: .txt, .md, .pdf, .docx</div>
      </div>
      <div>
