      ...
      {task && (
        <div className="space-y-4">
          ...
          <div>
            ...
            {form.description && /Из встречи:\s*(.+?)(?:\s*\(\d{4}-\d{2}-\d{2}\))?$/.test(form.description) && (() => {
              const match = form.description.match(/Из встречи:\s*(.+?)(?:\s*\((\d{4}-\d{2}-\d{2})\))?$/);
                ...
                <button
                  onClick={async () => {
                    try {
                      ...
                      if (found) {
                        window.location.href = `/meetings?open=${found.id}`;
                      }
                    } catch {}
                  }}
                  className="mb-2 text-xs text-indigo-500 hover:text-indigo-700 dark:text-indigo-400 dark:hover:text-indigo-300 underline cursor-pointer"
                >
