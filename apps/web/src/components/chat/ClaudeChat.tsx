...
export function ClaudeChat() {
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages([...newMsgs, { role: 'assistant', content: `Error: ${e instanceof Error ? e.message : 'Unknown'}` }]);
  ...
  return (
    <div className="flex flex-col h-full border-l bg-white">
      ...
      <div className="p-3 border-t flex gap-2">
        ...
        <button onClick={send} disabled={!input.trim() || loading} className="px-3 py-1.5 bg-indigo-600 text-white text-sm rounded-lg hover:bg-indigo-700 disabled:opacity-50">Send</button>
      </div>
    </div>
  );
}
