...
export function GanttPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="relative overflow-hidden h-full flex flex-col">
                ...
                return (
                  <div key={task.id} className="flex" style={{ height: ROW_HEIGHT }}>
                    ...
                    <div className="relative" style={{ width: TOTAL_DAYS * DAY_WIDTH }}>
                      <div
                        className="absolute top-1 rounded-md cursor-pointer hover:brightness-110 hover:-translate-y-0.5 hover:shadow-lg transition-all duration-200 active:scale-[0.98] flex items-start px-2 py-0.5 overflow-hidden"
                        style={{
                        ...
                        title={task.title}
                      >
                        <span className="text-[11px] text-white font-medium line-clamp-2 leading-tight">{task.title}</span>
                      </div>
