...
Паттерн — `'(parsers|search|projects|critical-paths|db)\.test\.ts$|/api\.test\.ts$'`: альтернатива `api` вынесена из общей группы отдельным звеном с границей `/api\.test\.ts$`. Внутри общей группы (`...|api)\.test\.ts$`) она матчила бы и `openapi.test.ts` — тот тоже оканчивается на «api.test.ts» — и молча выключала бы из прогона 13 тестов рекурсивного OpenAPI-валидатора, включая проверку на `nullable:`, запрещённый в 3.1. Итого суитов в `packages/api` — 36 (`jest --listTests`), с этим паттерном под фильтр попадают ровно 6, в прогоне остаётся 30, `openapi.test.ts` — среди них.
